...
def get_postgre_uri() -> str:
    ...
    if postgre_uri is None:
        raise ValueError("PostgreSQL DSN is missing. Set the PG_DSN environment variable.")
    return postgre_uri
...
def get_cdsapi_url() -> str:
    ...
    if cdsapi_url is None:
        raise ValueError("CDS API URL is missing. Set the CDSAPI_URL environment variable.")
    return cdsapi_url
...
def get_cdsapi_key() -> str:
    ...
    if cdsapi_key is None:
        raise ValueError("CDS API key is missing. Set the CDSAPI_KEY environment variable.")
    return cdsapi_key
